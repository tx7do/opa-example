...
func dumpData(ctx context.Context, store storage.Store, l *zap.Logger) error {
	...
	l.Info("data: ", zap.String("json", string(jsonData)))
...
func (s *State) evalQuery(
	...
	rs, err := rego.New(
		...
		rego.QueryTracer(tracer), // trace execution for debugging
	).Eval(ctx)
	if err != nil {
		return nil, err
	}
